	...
	public ResultSet getColumns(String catalog, String schemaPattern,
	  ...
	  if ( cursors.length == 0 ) {
	    ...
	    resultSet = new SQLDroidResultSet(cursors[0]);
		  ...
		  // TODO The implementation return only the resultset of the first cursor
		  // resultSet = new SQLDroidResultSet(new MergeCursor( cursors ));
		  resultSet = new SQLDroidResultSet(cursors[0]);
		  Log.e(" ********************* not implemented @ " + DebugPrinter.getFileName() + " line "
				  + DebugPrinter.getLineNumber() + " - MULTIPLE CURSOR RESULTSET - RETURNING ONLY THE RESULTSET OF THE FIRST CURSOR");
	  }
	  return resultSet;
	}
